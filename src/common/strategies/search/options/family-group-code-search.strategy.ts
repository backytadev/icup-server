...
@Injectable()
export class FamilyGroupCodeSearchStrategy implements SearchStrategy {
  async execute<T>({
    params,
    church,
    relations,
    ...
    if (familyGroups.length === 0) {
      throw new NotFoundException(
        `No se encontraron grupos familiares con este código: ${term} y con esta iglesia: ${
    ...
    if (data.length === 0) {
      throw new NotFoundException(
        `No se encontraron ${moduleName} con este código de grupo familiar ${term} y con esta iglesia: ${church ? church?.abbreviatedChurchName : 'Todas las iglesias'}`,
      );
    }
