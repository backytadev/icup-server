...
@Injectable()
export class OfferingIncomeByGroupCodeStrategy
  ...
  async execute({
    ...
    if (offeringIncome.length === 0) {
      throw new NotFoundException(
        `No se encontraron ingresos de ofrendas (${OfferingIncomeSearchTypeNames[searchType]}) con este código de grupo familiar: ${term} y con esta iglesia: ${church ? church.abbreviatedChurchName : 'Todas las iglesias'}`,
      );
    }
